...
from dataset_utils import processors, LazyDataset, bytes_to_id, _read_mentions
from modeling import BertForCorefLinking
...
MODEL_CLASSES = {
    'bert': (BertConfig, BertForCorefLinking, BertTokenizer),
}
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids':      batch[3],
                      ...
                      'labels': batch[2].type(torch.cuda.FloatTensor)}
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        if results["coref_accuracy"] > best_val_accuracy:
                            best_val_accuracy = results["coref_accuracy"]
...
def evaluate(args, model, tokenizer, prefix="", split=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, args.task_name, tokenizer, split=split, evaluate=True)
        ...
        logger.info("  Total eval batch size (w. parallel & distributed) = %d",
                       args.eval_batch_size * (torch.distributed.get_world_size() if args.local_rank != -1 else 1))
        eval_loss = 0.0
        nb_eval_steps = 0
        preds = None
        out_label_ids = None
        ...
        # Load cached vanilla linking model output scores
        ...
        tn = 0
        fn = 0
        surrogate_mention_id_map = {}

        for batch in tqdm(eval_dataloader, desc="Computing mention2mention scores"):
            model.eval()
            batch = tuple(t.to(args.device) for t in batch)

            with torch.no_grad():
                serialized_mention_id = batch[0].cpu().numpy().tolist()
                serialized_coref_mention_id = batch[1].cpu().numpy().tolist()
                inputs = {'input_ids':      batch[3],
                          'attention_mask': batch[4],
                          'token_type_ids': batch[5] if args.model_type in ['bert', 'xlnet'] else None,
                          'labels': batch[2].type(torch.cuda.FloatTensor)}
                ...
                coref_mention_ids = [[bytes_to_id(byte_set) for byte_set in example_coref_mention_id]
                                for example_coref_mention_id in serialized_coref_mention_id]

                threshold = 0.5
                scores = torch.sigmoid(scores)
                preds = (scores > threshold).cpu().numpy()
                labels = (batch[2].cpu().numpy() > 0.0)

                output_tuple = confusion_matrix(labels, preds).ravel()
                ...
                max_indices = torch.argmax(scores, 1).cpu().numpy()

                for i in range(len(mention_ids)):
                    if np.all(preds[i] == False):
                        surrogate_mention_id_map[mention_ids[i][0]] = mention_ids[i][0]
                    else:
                        surrogate_mention_id_map[mention_ids[i][0]] = coref_mention_ids[i][max_indices[i]]

        embed()
        exit()

        # Compute vanilla and coref accuracy
        vanilla_hits = 0
        coref_hits = 0
        total = len(mentions)
        for mention in mentions:
            surrogate_mention_id = surrogate_mention_id_map[mention['mention_id']]
            vanilla_linked_entity_id = max(mention_entity_scores[mention['mention_id']].items(), key=operator.itemgetter(1))[0]
            coref_linked_entity_id = max(mention_entity_scores[surrogate_mention_id].items(), key=operator.itemgetter(1))[0]
            if vanilla_linked_entity_id == mention['label_document_id']:
                vanilla_hits += 1
            if coref_linked_entity_id == mention['label_document_id']:
                coref_hits += 1


        result = {'vanilla_accuracy': vanilla_hits / total,
                  'coref_accuracy': coref_hits / total}
...
def load_and_cache_examples(args, task, tokenizer, split=None, evaluate=False):
    ...
    if os.path.exists(cached_indices_file) and not args.overwrite_cache:
        ...
        indices = processor.convert_examples_to_features(
            examples,
            documents,
            cached_examples_dir,
            args.max_seq_length,
        ...
        if args.local_rank in [-1, 0]:
            logger.info("Saving indices into cached file %s", cached_indices_file)
...
def main():
    ...
    parser.add_argument("--use_true_labels", action="store_true",
                        help="add ground truth entity to candidate set during evaluation")
    parser.add_argument("--loading_trained_model", action="store_true",
                        help="flag indicating know how to load the model")
    parser.add_argument("--mention_entity_scores_dir", default="", type=str,
                        help="pickle file containing vanilla linking scores")
    parser.add_argument("--coref_candidate_sets_dir", default="", type=str,
                        help="pickle file containing coref candidate sets")
    ...
    tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
                                                ...
                                                cache_dir=args.cache_dir if args.cache_dir else None)
    #if args.loading_trained_model:
    #    model = torch.load(os.path.join(args.model_name_or_path, 'model.pt'))
    #else:
    #    model = BertForCorefLinking(args.model_name_or_path, config, args.cache_dir)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, split='val')
