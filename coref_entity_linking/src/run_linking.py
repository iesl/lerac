...
from dataset_utils import processors, LazyDataset, bytes_to_id
from modeling import BertForLinking
...
MODEL_CLASSES = {
    'bert': (BertConfig, BertForLinking, BertTokenizer),
}
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.evaluate_during_training:
                        ...
                        if results["accuracy"] > best_val_accuracy:

                            best_val_accuracy = results["accuracy"]
...
def evaluate(args, model, tokenizer, prefix="", split=None):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, split=split, evaluate=True, use_true_labels=args.use_true_labels)
            ...
            with torch.no_grad():
                ...
                for i in range(scores.shape[0]):
                    for j in range(scores.shape[1]):
                        mention_entity_scores[mention_ids[i][j]][entity_ids[i][j]] = scores[i][j]

        hits = sum([v for _, v in hit_by_mention_id.items()])
        total = len(hit_by_mention_id.keys())
        accuracy = hits / total

        if args.dump_mention_entity_scores:
...
def load_and_cache_examples(args, task, tokenizer, split=None, evaluate=False, use_true_labels=False):
    ...
    if use_true_labels:
        label_mode = 'cheating-cand-gen'
    else:
        label_mode = 'realistic-cand-gen'

    split_desc = 'cached_{}_{}_{}_{}_{}'.format(
        cached_mode,
        label_mode,
    ...
    if os.path.exists(cached_indices_file) and not args.overwrite_cache:
        ...
        if split == 'train' and not evaluate:
            ...
            examples, documents = processor.get_eval_examples(
                    ...
                    args.num_candidates_per_example,
                    split, args.train_domains,
                    use_true_labels=use_true_labels) # TODO: letting this be true is cheating for the cheating experiment
        elif split == 'val':
            examples, documents = processor.get_eval_examples(
                    args.data_dir, args.num_candidates,
                    args.num_candidates_per_example,
                    split, args.val_domains,
                    use_true_labels=use_true_labels) # TODO: letting this be true is cheating for the cheating experiment
        elif split == 'test':
            examples, documents = processor.get_eval_examples(
                    args.data_dir, args.num_candidates,
                    args.num_candidates_per_example,
                    split, args.test_domains,
                    use_true_labels=use_true_labels) # TODO: letting this be true is cheating for the cheating experiment
        else:
            assert False
...
def main():
    ...
    parser.add_argument("--dump_mention_entity_scores", action='store_true',
                        help="Set this flag if you want to dump model scores for all splits.")
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, split='train', evaluate=False, use_true_labels=True)
    ...
    if args.dump_mention_entity_scores:
        ...
        for checkpoint in checkpoints:
            ...
            for split in ['train', 'val', 'test']:
                _ = evaluate(args, model, tokenizer, prefix=prefix, split=split)
