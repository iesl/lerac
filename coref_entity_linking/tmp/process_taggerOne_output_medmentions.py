...
TARGET_FILE = 'tagger_one-predicted_entities.txt'
PRED_FILE = 'tagger_one-target_entities.txt'
TRAIN_JSON_FILE = '../data/mm_st21pv_long_entities/mentions/train.json'
TEST_JSON_FILE = '../data/mm_st21pv_long_entities/mentions/test.json'
...
with open(TARGET_FILE, 'r') as f:
    ...
    for row in tsv_reader:
        target_cuids[(row[2], row[3], row[4])].append(
            row[5].replace('MESH:', '').replace('OMIM:', '').replace('UMLS:', '')
        )
